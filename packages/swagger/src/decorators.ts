import { TSwaggerResponseConfigValue, TSwaggerResponseOpts, getSwaggerMate } from './swagger.mate'
...
export function SwaggerResponse(code: number | TSwaggerResponseOpts, opts?: TSwaggerResponseOpts): MethodDecorator {
...
export function SwaggerRequestBody(opt: TSwaggerResponseOpts) {
    return getSwaggerMate().decorate((meta) => {
        ...
        const contentType = typeof (opt as { contentType: string }).contentType === 'string' ? (opt as { contentType: string }).contentType : 'application/json'
