...
import { TMoostZodType, TZodMetadata, getZodType, getZodTypeForProp, z } from '@moostjs/zod'
import { TZodParsed, parseZodType } from 'zod-parser'
import { TAny, TLogger } from 'common'
...
export function mapToSwaggerSpec(metadata: TControllerOverview[], options?: TSwaggerOptions, logger?: TLogger) {
    ...
    for (const controller of metadata) {
        ...
        for (const handler of controller.handlers) {
            ...
            let responses: TEndpointSpec['responses'] | undefined = undefined
            ...
            if (hmeta?.swaggerResponses) {
                for (const [code, responseConfigs] of Object.entries(hmeta.swaggerResponses)) {
                    ...
                    for (const [contentType, type] of Object.entries(responseConfigs)) {
                        let schema: TSwaggerSchema | undefined = undefined
                        let zt: z.ZodType | undefined = undefined
            ...
            for (let i = 0; i < handler.meta.params.length; i++) {
                ...
                if (paramMeta?.paramSource === 'BODY') {
                    ...
                    switch (parsed.$type) {
                        ...
                        case 'ZodEnum':
                        case 'ZodNativeEnum':
                        case 'ZodLiteral': contentType = 'text/plan'; break
...
function getSwaggerSchema(parsed: TZodParsed, forParam?: boolean): TSwaggerSchema | undefined {
    ...
    if (forParam) {
        ...
        switch (parsed.$type) {
            ...
            case 'ZodTuple':
                schema.type = 'array'
                schema.items = parsed.$inner.map(t => getSwaggerSchema(t)).filter(t => !!t) as TSwaggerSchema[]
                break
            case 'ZodObject':
                schema.type = 'object'
                schema.properties = {}
                schema.required = []
                if ((zodType as z.ZodObject<TAny>)._def.unknownKeys === 'passthrough') schema.additionalProperties = {}
            ...
            case 'ZodDiscriminatedUnion':
                schema.oneOf = parsed.$inner.map(t => getSwaggerSchema(t)).filter(t => !!t) as TSwaggerSchema[]
                break
            case 'ZodIntersection':
                schema.allOf = parsed.$inner.map(t => getSwaggerSchema(t)).filter(t => !!t) as TSwaggerSchema[]
                break
            case 'ZodLazy':
    ...
    if ((parsed.$ref as z.ZodString)._def.description as string) {
        schema.description = (parsed.$ref as z.ZodString)._def.description as string
    }
    if (parsed.$checks) {
...
function myParseZod(schema: TMoostZodType) {
    return parseZodType<TZodParsed>(schema)
}
