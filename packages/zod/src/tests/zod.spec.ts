...
import {
  ...
  ZodTestClass2,
  ZodTestClassArray,
  ZodTestClassCoerceDecorated,
  ZodTestClassCoerceDecoratedArray,
  ZodTestClassCoercePrimitive,
  ZodTestClassCoercePrimitiveArray,
  ZodTestClassCoerceTyped,
  ZodTestClassCoerceTypedArray,
  ZodTestClassOptionalArray,
  ZodTestClassRefine,
  ZodTestClassStringArray,
  ZodTestClassZod,
} from './zod.artifacts'
...
`)
  ...
  it('must validate with zod refine', async () => {
    expect(await validate({ myString: 'test string' }, ZodTestClassRefine)).toEqual({
      myString: 'test string',
    })
    expect(
      await validate(
        { myString: 'test string long string longer thatn 50 characters!!!' },
        ZodTestClassRefine,
        undefined,
        true
      )
...
`)
  ...
  it('must apply Coerce decorator for primitiveArray property', async () => {
    expect(
      await validate({ primitiveArray: [1, 5] }, ZodTestClassCoercePrimitiveArray, undefined, true)
    ).toEqual({
      success: true,
  ...
  it('must apply Coerce decorator for typedArray property', async () => {
    expect(
      await validate({ typedArray: [2, 5] }, ZodTestClassCoerceTypedArray, undefined, true)
    ).toEqual({
      success: true,
      data: { typedArray: ['2', '5'] },
    })
  ...
  it('must apply Coerce decorator for decoratedArray property', async () => {
    expect(
      await validate({ decoratedArray: [3, 5] }, ZodTestClassCoerceDecoratedArray, undefined, true)
    ).toEqual({
      success: true,
  ...
  it('must figure out that the type is array just by array runtime type', async () => {
    expect(
      await validate({ a: ['first', 'last'] }, ZodTestClassStringArray, undefined, true)
    ).toEqual({
      success: true,
      data: { a: ['first', 'last'] },
    })
