...
import { TInputs, TPrompts } from './types'
        ...
        const results = await prompts<keyof TPrompts | 'overwriteChecker'>(
            [
                ...
                },
                {
                    name: 'overwrite',
                    type: () =>
                        canSkipEmptying(predefined.targetDir as string) || inputs.force
                            ? null
                            : 'confirm',
                    message: () => {
                        const dirForPrompt =
                            predefined.targetDir === '.'
                                ? 'Current directory'
                                : `Target directory "${predefined.targetDir as string}"`
                ...
                {
                    name: 'packageName',
                    type: () => (isValidPackageName(predefined.targetDir as string) ? null : 'text'),
                    message: 'Package name:',
                    initial: () => toValidPackageName(predefined.targetDir as string),
                    validate: (dir: string) =>
