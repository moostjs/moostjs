...
export function Validate<T = unknown>(
    validator: TValidoFn<T>
): PropertyDecorator & ParameterDecorator {
    return getMoostMate().decorate(
        isArrayItemValidator ? 'validatorsOfItem' : 'validators',
        validator,
        true
    )
}
...
export function IsArray<T = unknown>(
    opts?: TValidateArrayOptions<T>
): PropertyDecorator & ParameterDecorator {
    const mate = getMoostMate()
    const decorators = [mate.decorate('arrayType', opts || true)] as (
        | PropertyDecorator
        | ParameterDecorator
    )[]
    if (opts?.itemValidators && !isArrayItemValidator) {
        ...
        throw new Error(
            'IsArray validator is not supported inside of array type'
        )
    }
...
export function IsString(
    ...args: Parameters<typeof validoIsString>
): PropertyDecorator & ParameterDecorator {
    return Validate(validoIsString(...args))
}

export function IsNumber(
    ...args: Parameters<typeof validoIsNumber>
): PropertyDecorator & ParameterDecorator {
    return Validate(validoIsNumber(...args))
}

export function IsBoolean(
    ...args: Parameters<typeof validoIsBoolean>
): PropertyDecorator & ParameterDecorator {
