...
description: >-
  ...
  build decorator-driven TypeScript apps for HTTP, CLI, WebSocket, Workflow or
  custom events. Covers controllers, subclassing (@Inherit),
  handler/param/response decorators, DI,
  interceptors, pipes, auth guards, framework errors, metadata decorators,
  Wooks composables, adapter authoring, workflow outlets/state,
  WebSocket rooms/testing, logging, boot/teardown hooks (@MoostInit,
  @MoostDispose, app.dispose(), graceful shutdown, hot reload leaks connections),
  the Vite plugin (dev/build, middleware, SSR/SPA, deployment, SSR externalization
  splits: AtscriptDbView instanceof, empty table instead of view),
  event context/scoping, Swagger/OpenAPI, and OTel
...
| --------------------------------------------------------------------- | ------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Core concepts, Moost class, lifecycle, registration, app init/dispose | [core.md](references/core.md)                                | New project, multi-adapter setup, `app.init()`, `@MoostInit`/`@InjectMoost`, boot-time setup, Moost subclass, `app.dispose()`/`@MoostDispose`, graceful shutdown, `disposeOnSignals()` (SIGTERM/SIGINT handler, MaxListenersExceededWarning on dev reload, Ctrl-C disposes the wrong app), resources leaking across hot reloads, duplicate timers/jobs after reload                                                                                                                                                                                                                                               |
...
| Vite plugin: dev/build, middleware, SSR/SPA, deploy                   | [vite.md](references/vite.md)                                | `moostVite`, `vite.config.ts`, `createSSRServer`, `ssrEntry`/`serverEntry`, `ssr.noExternal`/`ssr.external`, `ssrExternalCheck`, `dist/server`, HMR, "undefined `useRequest()` in prod", "Cannot read properties of undefined (reading 'headers')", externalized dep calling wooks composables, dev server serves a half-booted app / some API routes return index.html after an edit / `app.init()` error not showing in dev / "502 Moost app failed to load" / does the plugin wait for `init()`, SSR render contract / per-page head-SEO / status / `ssrHead` / `TSSRRenderResult` / `TSSRRenderContext` / `ssrFetchForwarding` (SSR viewer identity) |
